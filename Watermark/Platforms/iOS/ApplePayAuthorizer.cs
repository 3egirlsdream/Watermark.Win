    ...
    public class ApplePayAuthorizer : PKPaymentAuthorizationViewControllerDelegate, IApplePayAuthorizer
    {
        ...
        public void AuthorizePayment(double Amount)
        {
            ...
            try
            {
                ...
                paymentRequest.MerchantIdentifier = "merchant.com.top.thankful.watermark.ios";
                ...
                paymentRequest.PaymentSummaryItems = MakeSummaryItems((int)Amount);
        ...
        PKPaymentSummaryItem[] MakeSummaryItems(int seq)
        {
            Dictionary<int, Tuple<string, string>> dic = new Dictionary<int, Tuple<string, string>>
            {
                [28] = Tuple.Create("年度会员", "watermarkyearvip"),
                [18] = Tuple.Create("季度会员", "watermark3monthvipservice"),
                [8] = Tuple.Create("月度会员", "watermark1monthvip")
            };
            var items = new List<PKPaymentSummaryItem>();
            var b = dic.TryGetValue(seq, out Tuple<string, string> v);
            if (!b) return new PKPaymentSummaryItem[] { };
            var productSummaryItem = PKPaymentSummaryItem.Create(v.Item2, new NSDecimalNumber(PayAmount));
            items.Add(productSummaryItem);

            var totalSummaryItem = PKPaymentSummaryItem.Create(v.Item1, productSummaryItem.Amount);
