...
public sealed class MacColorPreviewValidator
{
    ...
    public MacColorPreviewValidationRequest CreateRequest()
    {
        const int width = 32;
        const int height = 24;
        ...
        var hslBands = Enum.GetValues<WMHslBand>();
        for (var index = 0; index < hslBands.Length; index++)
            manual.Hsl[hslBands[index]] = new WMHslAdjustment
            {
        ...
        automatic.Normalize();
        manual.Normalize();
        var lut = CreateValidationLut(17);
        var generated = new WMGeneratedColorLook(automatic, lut, $"validation-v{WMColorPipelineVersion.Current}");
        using var expected = WMHighPrecisionColorPipeline.ApplyPreviewReference(source, generated, manual);

        return new MacColorPreviewValidationRequest(
            width,
            height,
            sourceBytes,
            ReadRgba(expected),
            MacColorPreviewLook.From(generated),
            MacColorPreviewParameters.From(manual));
    }

    public MacColorPreviewValidationResult Evaluate(MacColorPreviewValidationRequest request, byte[] actualRgba)
    {
        var requiredLength = request.Width * request.Height * 4;
        if (actualRgba.Length != requiredLength)
            return new MacColorPreviewValidationResult(false, double.MaxValue, double.MaxValue, 255, 0,
        ...
        return new MacColorPreviewValidationResult(
            passed, averageDeltaE, maximumDeltaE, maximumChannelError, passRate, reason);
    }
    ...
    {
        var bitmap = new SKBitmap(new SKImageInfo(width, height, SKColorType.Rgba8888, SKAlphaType.Unpremul, SKColorSpace.CreateSrgb()));
        for (var y = 0; y < height; y++)
        for (var x = 0; x < width; x++)
        {
            var offset = (y * width + x) * 4;
            bitmap.SetPixel(x, y, new SKColor(rgba[offset], rgba[offset + 1], rgba[offset + 2], rgba[offset + 3]));
        }
        return bitmap;
    }
