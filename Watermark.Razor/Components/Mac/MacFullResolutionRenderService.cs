...
public sealed record MacFullResolutionRenderRequest(
    MacRenderPlan Plan,
    string OutputPath,
...
public sealed class MacFastJpegExportService
{
    ...
    public MacFastJpegExportService(
    ...
    public async Task RenderAsync(
        MacFullResolutionRenderRequest request,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            for (var index = 0; index < request.Plan.Steps.Count; index++)
            {
                ...
                {
                    var settings = (WMTemplateOperationSettings)MacFullResolutionRenderService.DeserializeSettings(operation);
    ...
    private static string CreateCacheKey(MacFullResolutionRenderRequest request)
    {
        ...
        var builder = new StringBuilder()
            .Append("mac-fast-jpeg-v").Append(PipelineVersion).Append('|')
...
public sealed class MacFullResolutionRenderService
{
    private readonly IWMProcessingScheduler scheduler;
    ...
    private readonly MacFastJpegExportService fastJpegExportService;

    public MacFullResolutionRenderService(
        ...
        MacFastJpegExportService? fastJpegExportService = null)
    {
        this.scheduler = scheduler;
        ...
        this.fastJpegExportService = fastJpegExportService
            ?? new MacFastJpegExportService(new WMTemplateRenderer(new WatermarkHelper()), colorProcessor, scheduler);
    }
    ...
    private bool CanRenderHighPrecision(MacFullResolutionRenderRequest request)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var replaySteps = useCommittedVersion ? Array.Empty<MacRenderPlanStep>() : request.Plan.Steps;
        ...
        CancellationToken cancellationToken)
    {
        ...
        using var reader = new WM16FileReader(currentPath);
        var bounds = resolution == "1080" ? (Width: 1920, Height: 1080) : (Width: 3840, Height: 2160);
        var scale = Math.Min(1f, Math.Min(bounds.Width / (float)reader.Width, bounds.Height / (float)reader.Height));
